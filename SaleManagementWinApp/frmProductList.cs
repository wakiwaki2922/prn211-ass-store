...
namespace SaleManagementWinApp
{
    public partial class frmProductList : Form
    {
        ...
        {
            List<FlowerBouquet> productList;
            if (isAdmin)
            {
            ...
            {
                productList = productRepository.GetAllProduct().Where(p => p.UnitsInStock > 0).ToList();
            }

            dgvProductList.DataSource = productList.Select(c => new
            {
                c.FlowerBouquetId,
